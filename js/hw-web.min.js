...
},notfound:function(){ruto.go("/")}}).add("/","home").add("/about","about").add(/^\/item\/(\d+)$/i,"comments",function(a,b){f.comments.render(b)})}(window),function(a){if(a.addEventListener){var b,c=a.document,d=c.body,e={},f=function(){return a.pageYOffset||"CSS1Compat"===c.compatMode&&c.documentElement.scrollTop||d.scrollTop||0},g=function(){e[location.hash.slice(1)]=f()};a.addEventListener("scroll",function(){clearTimeout(b),b=setTimeout(g,500)},!1),ruto.config({on:function(){var b=location.hash.slice(1);
a.scrollTo(0,e[b]||0),e[b]=f()}});var h=c.head||c.getElementsByTagName("head")[0],i=function(){var a=window.innerHeight,b=$("view-height");b||(b=c.createElement("style"),b.id="view-height",h.appendChild(b)),window.innerWidth>=788&&(a*=.85),b.textContent=".view>.scroll{min-height: "+a+"px}"};if(a.addEventListener("resize",i,!1),a.addEventListener("orientationchange",i,!1),i(),ibento("#view-home-refresh","click",hw.news.reload),ibento("#view-home .more-link","click",function(a,b){hw.news.more(b)}),ibento("button.comments-toggle","click",function(a,b){hw.comments.toggle(b)
}),ibento("section.comments li>a.more-link","click",function(a,b){hw.comments.more(b)}),ibento("#view-comments .load-error button","click",hw.comments.reload),/Mobile;.*Firefox/.test(navigator.userAgent)&&navigator.mozApps){var j=navigator.mozApps.getSelf();j.onsuccess=function(){j.result&&ibento('a[href]:not([href^="#"])',"click",function(a,b){a.preventDefault(),window.open(b.href,"browser")})}}window.onload=hw.init}}(window);
/*
